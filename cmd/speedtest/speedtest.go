...
var speedtestCmd = &ffcli.Command{
	Name:       "speedtest",
	ShortUsage: "speedtest [-h <host:port>] [-s] [-r] [-t <test duration>]",
...
func runSpeedtest(ctx context.Context, args []string) error {
	...
	if !strings.Contains(speedtestArgs.host, ":") {
	...
	if speedtestArgs.testDuration < speedtest.MinDuration || speedtestArgs.testDuration > speedtest.MaxDuration {
		return fmt.Errorf("test duration must be within %v and %v.\n", speedtest.MinDuration, speedtest.MaxDuration)
	}

	var dir speedtest.Direction
